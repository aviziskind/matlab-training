    ...
    for (conn_i = 0; conn_i < nConnections; conn_i++) {
        ...
        for (i = 0; i < h_out; i++) {      // go through all output positions
            for (j = 0; j < w_out; j++) {
                ...
                pdYout[i + j*h_out + k*h_out*w_out] = pdYout[i + j*h_out + k*h_out*w_out] + 
                        pdBias[k] + weight_conv_input;
...
//    pdYout = (double***) mxCalloc(h_out * w_out * nOutputPlanes);
    ...
    /// ------------------- Yout (OUTPUT)----------        
    ndims_out = 3;
    dims_out = mxCalloc(ndims_out, sizeof(mwSize));
    dims_out[0] = h_out;
    dims_out[1] = w_out;
    dims_out[2] = nOutputPlanes;
